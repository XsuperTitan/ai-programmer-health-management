...
export function generatePlanFromKeywords(keywords: string[]): PlanDay[] {
  const habits = keywords.slice(0, 7).map((kw, i) => habitFromKeyword(kw, i))
  const plan: PlanDay[] = [
  ...
  const session = getSession()
  session.selectedKeywords = keywords.slice(0, 7)
  session.plan = plan
  session.planSource = 'customize'
  saveSession(session)
  return plan
}

export function needsPlanOverrideConfirm(): boolean {
  const session = getSession()
  return session.plan.length > 0 && session.planSource === 'quiz'
}
